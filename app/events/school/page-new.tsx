...
export default function ColorMixerPage() {
  ...
  const reverseMixingRules: { [key: string]: [string, string] } = useMemo(
    () => ({
      ...
      APRICOT: ["ORANGE", "WHITE"],
      EGGPLANT: ["BLUE", "PURPLE"],
  ...
  // Find mixing recipe for a target color
  const findMixingRecipe = (targetColor: string): [string, string] | null => {
    const upperTarget = targetColor.toUpperCase();
    return reverseMixingRules[upperTarget] || null;
  };

  // Handle finding mix
  const handleFindMix = () => {
    if (!targetColorInput.trim()) {
      setFeedbackMessage("Please enter a color name.");
      setFoundMix(null);
      return;
    }

    const recipe = findMixingRecipe(targetColorInput.trim());
    if (recipe) {
      setFoundMix({ color1: recipe[0], color2: recipe[1] });
      setFeedbackMessage(
        `To create ${targetColorInput.trim().toUpperCase()}, mix ${
          recipe[0]
        } and ${recipe[1]}!`
      );
    } else {
      setFoundMix(null);
      setFeedbackMessage(
        `Sorry, I don't know how to make ${targetColorInput
          .trim()
          .toUpperCase()}. Try a different color!`
      );
    }
  };

  // Autocomplete logic - find a single suggested color
  useEffect(() => {
    if (targetColorInput.trim().length < 2) {
      setSuggestedColor("");
      return;
    }

    const input = targetColorInput.trim().toLowerCase();
    const suggestion = allColorsWithHex.find((color) =>
      color.name.toLowerCase().startsWith(input)
    );

    if (suggestion && suggestion.name.toLowerCase() !== input) {
      setSuggestedColor(suggestion.name);
    } else {
      setSuggestedColor("");
    }
  }, [targetColorInput, allColorsWithHex]);
      ...
      <main className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-600 max-w-2xl mx-auto">
            Discover how to create beautiful colors by mixing primary colors!
            Enter a color name below to see what you need to mix.
          </p>
        </div>
            ...
            <div className="relative">
              ...
              {suggestedColor && suggestedColor !== targetColorInput && (
                <div className="absolute inset-0 flex items-center">
                  <span className="text-xl text-gray-900 font-normal ml-[calc(50%+0.5rem)]">
                    <button
                      onClick={() => {
                        setTargetColorInput(suggestedColor.toLowerCase());
                        setFeedbackMessage("");
                      }}
