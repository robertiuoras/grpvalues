...
export default function ColorMixerPage() {
  ...
  const [foundMix, setFoundMix] = useState<{ color1: string; color2: string } | null>(null);
  ...
  const handleFindMix = () => {
    ...
    if (reverseMixingRules[cleanedTargetColor]) {
      ...
      const availableMixableColorNames = Object.keys(reverseMixingRules).sort().join(', ');
      setFeedbackMessage(`Sorry, I don't know how to make ${cleanedTargetColor}. Try one of these mixable colors: ${availableMixableColorNames}.`);
      setFoundMix(null);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-purple-100 flex items-center justify-center p-4 sm:p-6 lg:p-8 font-inter">
      <main className="max-w-xl mx-auto bg-white bg-opacity-95 rounded-3xl shadow-2xl p-8 sm:p-12 lg:p-14 border border-blue-100 text-center relative overflow-hidden animate-float">
        <h1 className="text-4xl sm:text-5xl font-extrabold text-gray-800 leading-tight mb-8 drop-shadow-md relative z-10">
          Chemistry
        </h1>

        <div className="mb-8 relative z-10">
          <label htmlFor="targetColor" className="block text-xl font-semibold text-gray-700 mb-4">
        ...
        {feedbackMessage && (
          <p className={`relative z-10 text-lg font-medium mt-6 p-4 rounded-xl
            ${foundMix ? 'bg-green-100 text-green-800 border-green-300' : 'bg-red-100 text-red-800 border-red-300'}
            border shadow-md`}>
            {feedbackMessage}
          </p>
        )}
        ...
        {foundMix && (
          <div className="relative z-10 mt-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div
              ...
              style={{ backgroundColor: colorToHex[foundMix.color1] || '#D1D5DB', color: allColorsWithHex.find(c => c.name === foundMix.color1)?.text || 'black' }}
            >
              {foundMix.color1}
            ...
            <div
              className="p-6 rounded-xl shadow-xl border border-gray-300 text-2xl font-bold flex items-center justify-center h-32 transition-transform duration-200 hover:scale-105 relative overflow-hidden group"
              style={{ backgroundColor: colorToHex[foundMix.color2] || '#D1D5DB', color: allColorsWithHex.find(c => c.name === foundMix.color2)?.text || 'black' }}
            >
              {foundMix.color2}
              <div className="absolute inset-0 bg-white opacity-0 group-hover:opacity-10 transition-opacity duration-200"></div>
            </div>
      ...
      {/* Custom CSS for the new float animation */}
      <style jsx>{`
        @keyframes float {
          0% { transform: translateY(0px); }
          50% { transform: translateY(-8px); } /* Increased float distance */
          100% { transform: translateY(0px); }
        }
        .animate-float {
          animation: float 4s ease-in-out infinite; /* Adjusted duration */
        }
      `}</style>
    </div>
  );
